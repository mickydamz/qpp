...
import 'package:qpp/theme4/utils/T4Colors.dart';
import 'package:vector_math/vector_math_64.dart' show Vector3;

enum LearnerBottomNavigationBarType {
  /// The [LearnerBottomNavigationBar]'s [LearnerBottomNavigationBarItem]s have fixed width.
  fixed,

  /// The location and size of the [T4BottomNavigationBar] [LearnerBottomNavigationBarItem]s
  /// animate and labels fade in when they are tapped.
  shifting,
}

class LearnerBottomNavigationBar extends StatefulWidget {
  LearnerBottomNavigationBar({
    Key key,
    @required this.items,
    this.onTap,
    this.currentIndex = 0,
    this.elevation = 8.0,
    LearnerBottomNavigationBarType type,
    Color fixedColor,
  ...
  final List<LearnerBottomNavigationBarItem> items;

  /// Called when one of the [items] is tapped.
  ///
  /// The stateful widget that creates the bottom navigation bar needs to keep
  /// track of the index of the selected [LearnerBottomNavigationBarItem] and call
  /// `setState` to rebuild the bottom navigation bar with the new [currentIndex].
  final ValueChanged<int> onTap;

  /// The index into [items] for the current active [LearnerBottomNavigationBarItem].
  final int currentIndex;

  /// The z-coordinate of this [LearnerBottomNavigationBar].
  ///
  ...
  /// Defines the layout and behavior of a [LearnerBottomNavigationBar].
  ///
  /// See documentation for [LearnerBottomNavigationBarType] for information on the
  /// meaning of different types.
  final LearnerBottomNavigationBarType type;
  ...
  /// The color of the [LearnerBottomNavigationBar] itself.
  ///
  /// If [type] is [LearnerBottomNavigationBarType.shifting] and the
  /// [items]s, have [LearnerBottomNavigationBarItem.backgroundColor] set, the [item]'s
  /// backgroundColor will splash and overwrite this color.
  final Color backgroundColor;

  /// The size of all of the [LearnerBottomNavigationBarItem] icons.
  ///
  /// See [LearnerBottomNavigationBarItem.icon] for more information.
  final double iconSize;

  /// The color of the selected [LearnerBottomNavigationBarItem.icon] and
  /// [LearnerBottomNavigationBarItem.label].
  ///
  /// If null then the [ThemeData.primaryColor] is used.
  final Color selectedItemColor;

  /// The color of the unselected [LearnerBottomNavigationBarItem.icon] and
  /// [LearnerBottomNavigationBarItem.label]s.
  ///
  /// If null then the [TextTheme.caption]'s color is used.
  final Color unselectedItemColor;

  /// The size, opacity, and color of the icon in the currently selected
  /// [LearnerBottomNavigationBarItem.icon].
  ///
  ...
  /// [LearnerBottomNavigationBarItem.icon]s
  ///
  /// If this is not provided, the size will default to [iconSize], the color
  ...
  /// The [TextStyle] of the [LearnerBottomNavigationBarItem] labels when they are
  /// selected.
  final TextStyle selectedLabelStyle;

  /// The [TextStyle] of the [LearnerBottomNavigationBarItem] labels when they are not
  /// selected.
  final TextStyle unselectedLabelStyle;

  /// The font size of the [LearnerBottomNavigationBarItem] labels when they are selected.
  ///
  ...
  /// The font size of the [LearnerBottomNavigationBarItem] labels when they are not
  /// selected.
  ///
  ...
  /// Whether the labels are shown for the selected [LearnerBottomNavigationBarItem].
  final bool showUnselectedLabels;

  /// Whether the labels are shown for the unselected [LearnerBottomNavigationBarItem]s.
  ...
  static LearnerBottomNavigationBarType _type(
    LearnerBottomNavigationBarType type,
    List<LearnerBottomNavigationBarItem> items,
  ) {
    if (type != null) {
      return type;
    }
    return items.length <= 3 ? LearnerBottomNavigationBarType.fixed : LearnerBottomNavigationBarType.shifting;
  }
  ...
  static bool _defaultShowUnselected(LearnerBottomNavigationBarType type) {
    switch (type) {
      case LearnerBottomNavigationBarType.shifting:
        return false;
  ...
  @override
  _LearnerBottomNavigationBarState createState() => _LearnerBottomNavigationBarState();
}
...
class _BottomNavigationTile extends StatelessWidget {
  ...
  final LearnerBottomNavigationBarType type;
  final LearnerBottomNavigationBarItem item;
...
class _LearnerBottomNavigationBarState extends State<LearnerBottomNavigationBar> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(LearnerBottomNavigationBar oldWidget) {
...
class _Circle {
  ...
  final _LearnerBottomNavigationBarState state;
  final int index;
  final Color color;
...
class LearnerBottomNavigationBarItem {
  /// Creates an item that is used with [LearnerBottomNavigationBar.items].
  ///
  /// The argument [icon] should not be null and the argument [title] should not be null when used in a Material Design's [LearnerBottomNavigationBar].
  const LearnerBottomNavigationBarItem({
    @required this.icon,
    this.title,
    Widget activeIcon,
    this.backgroundColor,
  ...
  /// [LearnerBottomNavigationBarType.shifting].
  final String icon;
  ...
  /// See also:
  ///
  ///  * [LearnerBottomNavigationBarItem.icon], for a description of how to pair icons.
  final String activeIcon;

  /// The title of the item. If the title is not provided only the icon will be shown when not used in a Material Design [LearnerBottomNavigationBar].
  final Widget title;

  /// The color of the background radial animation for material [LearnerBottomNavigationBar].
  ///
  /// If the navigation bar's type is [LearnerBottomNavigationBarType.shifting], then
  /// the entire bar is flooded with the [backgroundColor] when this item is
  /// tapped. This will override [LearnerBottomNavigationBar.backgroundColor].
  ///
